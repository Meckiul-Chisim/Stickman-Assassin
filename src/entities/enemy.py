...
class Enemy:
    def __init__(self, x, y, audio, effects, variant="guard"):
        ...
        self.health = settings.ENEMY_MAX_HEALTH * (2 if variant == "elite" else 1)
    ...
    def update(self, player):
        ...
        self.anim_time += 0.15
        distance = abs(player.x - self.x); was_alert = self.alert
        ...
        if distance <= settings.ENEMY_ATTACK_RANGE:
            if self.attack_cooldown == 0:
                self.attack_windup = settings.ENEMY_ATTACK_WINDUP; self.attack_cooldown = settings.ENEMY_ATTACK_COOLDOWN
        else:
            self.x += settings.ENEMY_SPEED if player.x > self.x else -settings.ENEMY_SPEED
    ...
    def take_damage(self, amount, hit_x=None, hit_y=None):
        ...
        self.audio.play("hit")
        if self.health <= 0:
            self.health = 0; self.dead = True; self.death_timer = 34
    ...
    def draw(self, surface):
        ...
        elite = self.variant == "elite"
        base = (118, 128, 142) if not elite else (76, 98, 130)
        hi = (182, 193, 207) if not elite else (130, 176, 226)
        accent = settings.COLOR_ACCENT_RED if self.alert else ((168, 88, 112) if elite else (103, 116, 132))
        dark = (9, 13, 20); stride = int(math.sin(self.anim_time) * 4) if self.attack_windup == 0 else 0
        head, shoulder, hip = bottom - 75, bottom - 57, bottom - 33
        ...
        pygame.draw.polygon(surface,base,[(x-14,shoulder),(x+14,shoulder),(x+11,hip),(x-11,hip)])
        pygame.draw.line(surface,hi,(x-9,shoulder+7),(x+9,shoulder+7),3); pygame.draw.circle(surface,accent,(x,shoulder+16),3)
        if self.attack_windup > 0:
            pygame.draw.line(surface,hi,(x+8,shoulder+2),(x+27,shoulder-10),6); pygame.draw.line(surface,hi,(x-7,shoulder+4),(x-22,shoulder+15),5)
            pygame.draw.line(surface,(222,228,235),(x+27,shoulder-10),(x+51,shoulder-30),4)
        else:
        ...
        bw = 58 if elite else 48; hp = int(bw*self.health/self.max_health)
        pygame.draw.rect(surface,settings.COLOR_PANEL,(x-bw//2,bottom-103,bw,6),border_radius=3); pygame.draw.rect(surface,settings.COLOR_ACCENT_RED if self.alert else hi,(x-bw//2,bottom-103,hp,6),border_radius=3)
    ...
    def _draw_death_fade(self, surface, x, bottom):
        ...
        pygame.draw.circle(ghost,color,(cx,cy-25),15,3); pygame.draw.line(ghost,color,(cx,cy-10),(cx,cy+25),5)
        pygame.draw.line(ghost,color,(cx,cy+25),(cx-22,cy+55),5); pygame.draw.line(ghost,color,(cx,cy+25),(cx+22,cy+55),5)
        pygame.draw.line(ghost,(*settings.COLOR_ACCENT_RED,alpha),(cx,cy+5),(cx+35,cy-25),3); surface.blit(ghost,(x-55,bottom-110))
