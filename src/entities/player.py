...
class Player:
    ...
    def handle_input(self, keys, actions=None):
        if not self.alive:
            return
        actions = actions or {}
        dx = 0
        left = keys[pygame.K_a] or keys[pygame.K_LEFT] or actions.get("left", False)
        right = keys[pygame.K_d] or keys[pygame.K_RIGHT] or actions.get("right", False)
        if left:
        ...
        wants_jump = keys[pygame.K_w] or keys[pygame.K_UP] or keys[pygame.K_SPACE] or actions.get("jump", False)
        if wants_jump and self.on_ground:
            self.velocity_y = settings.PLAYER_JUMP_POWER
            self.on_ground = False
            self.audio.play("jump")

    def update(self, keys, actions=None):
        if not self.alive:
            return
        actions = actions or {}
        self.anim_time += 0.18 if self.on_ground else 0.28
        self.handle_input(keys, actions)
    ...
    def dash(self):
        if not self.alive or self.dash_cooldown > 0 or self.attacking:
            return
        self.x = max(25, min(settings.WIDTH - 25, self.x + self.facing * settings.DASH_DISTANCE))
        self.dash_cooldown = settings.DASH_COOLDOWN
        self.invuln_timer = max(self.invuln_timer, 14)
        self.hidden = False; self.stealth_timer = 0
        self.effects.dust_step(self.x - self.facing * 30, self.y)
